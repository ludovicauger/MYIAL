...
!   P Brousseau Jan-2023 allow multi-IAU
! End Modifications
!      ----------------------------------------------------------------
...
USE YOMCT0        , ONLY : NCONF, &
 ...
 &                         LSLAG,LOBS,NFRCORM,L_OOPS,LNHEE,LNHQE,LCONSERV,LCORWAT,LNHDYN
...
USE YOMIAU   , ONLY : LIAU, NSTARTIAU, TSTARTIAU, &
 &  NSTOPIAU, TSTOPIAU, NCIAU, NIAU
!      ----------------------------------------------------------------
...
! - IFPDITS : ARRAY CONTAINING FULLPOS NORMS TIME STEPS
INTEGER(KIND=JPIM), ALLOCATABLE :: IFPDITS(:)
...
REAL(KIND=JPRB),ALLOCATABLE :: ZDIVA(:,:),ZDIVS(:,:),ZVORA(:,:),ZVORS(:,:),ZTA(:,:),ZTS(:,:)
REAL(KIND=JPRB),ALLOCATABLE :: ZSPS(:),ZSPA(:),ZUBS(:),ZVBS(:),ZUBA(:),ZVBA(:)
REAL(KIND=JPRB),ALLOCATABLE :: ZSP3DA(:,:,:),ZSP3DS(:,:,:),ZSP2DA(:,:),ZSP2DS(:,:),ZSP1DA(:,:),ZSP1DS(:,:)
REAL(KIND=JPRB),ALLOCATABLE :: ZGFLS(:,:,:,:),ZGFLT1S(:,:,:,:),ZGFLPTS(:,:,:,:)
REAL(KIND=JPRB),ALLOCATABLE :: ZGFLA(:,:,:,:),ZGFLT1A(:,:,:,:),ZGFLPTA(:,:,:,:)
REAL(KIND=JPRB),ALLOCATABLE :: ZGMVS(:,:,:,:),ZGMVSS(:,:,:),ZGMVT1S(:,:,:,:),ZGMVT1SS(:,:,:)
REAL(KIND=JPRB),ALLOCATABLE :: ZGMVA(:,:,:,:),ZGMVSA(:,:,:),ZGMVT1A(:,:,:,:),ZGMVT1SA(:,:,:)
...
ASSOCIATE(YDGFL5=>YDMTRAJ%YRGFL5,YDGMV5=>YDMTRAJ%YRGMV5, YDGFL=>YDFIELDS%YRGFL,YDGMV=>YDFIELDS%YRGMV, &
 ...
 & YDNAMFPIOS=>YDFPOS%YFPIOH%YNAMFPIOS, &
...
ENDIF
WRITE(6,*) 'DEBUG CNT4 AVANT STEPX',__LINE__
CALL FLUSH(6)
...
ENDIF
WRITE(6,*) 'DEBUG CNT4 APRES STEPX',__LINE__
CALL FLUSH(6)
...
IF (PRESENT(YDFPOS)) THEN
  IF (YDFPOS%YFPCNT%NFRFPOS /= 0) THEN
    ...
    ENDIF
  ENDIF
  IF (YDNAMFPIOS%NFPWRITE==0 .OR. YDNAMFPIOS%NFPWRITE==1) THEN
    I1FPDI=1
  ELSE
    I1FPDI=0
  ENDIF
  IF (I1FPDI==1 .AND. YDNAMFPIOS%NFRFPDI /= 0) THEN
    ALLOCATE(IFPDITS(NSTART:YDMODEL%YRML_GCONF%YRRIP%NSTOP/YDNAMFPIOS%NFRFPDI))
    IF (ABS(YDNAMFPIOS%NFPDITSMIN(0)) > 0) THEN
      CALL MONIO_T(NSTART,YDRIP,IFPDITS(NSTART:),I1FPDI,YDNAMFPIOS%NFRFPDI,YDNAMFPIOS%NFPDITS, &
       & KN___TSMIN=YDNAMFPIOS%NFPDITSMIN)
    ELSE
      CALL MONIO_T(NSTART,YDRIP,IFPDITS(NSTART:),I1FPDI,YDNAMFPIOS%NFRFPDI,YDNAMFPIOS%NFPDITS)
    ENDIF
  ENDIF
...
IF (NPRINTLEV >= 1) THEN
  ...
  IF (PRESENT(YDFPOS)) THEN
    ...
    ENDIF
    IF (I1FPDI==1 .AND. YDNAMFPIOS%NFRFPDI /= 0) THEN
      WRITE(NULOUT,'('' FULLPOS NORMS EVENTS, IFPDITS '')')
      WRITE(NULOUT,'(40I2)')IFPDITS
    ENDIF
  ENDIF
...
ALLOCATE(ZGFLS(SIZE(YDFIELDS%YRGFL%GFL,1),SIZE(YDFIELDS%YRGFL%GFL,2),SIZE(YDFIELDS%YRGFL%GFL,3),SIZE(YDFIELDS%YRGFL%GFL,4)))
ALLOCATE(ZGFLA(SIZE(YDFIELDS%YRGFL%GFL,1),SIZE(YDFIELDS%YRGFL%GFL,2),SIZE(YDFIELDS%YRGFL%GFL,3),SIZE(YDFIELDS%YRGFL%GFL,4)))
ALLOCATE(ZGFLT1S(SIZE(YDFIELDS%YRGFL%GFLT1,1),SIZE(YDFIELDS%YRGFL%GFLT1,2),SIZE(YDFIELDS%YRGFL%GFLT1,3),SIZE(YDFIELDS%YRGFL%GFLT1,4)))
ALLOCATE(ZGFLT1A(SIZE(YDFIELDS%YRGFL%GFLT1,1),SIZE(YDFIELDS%YRGFL%GFLT1,2),SIZE(YDFIELDS%YRGFL%GFLT1,3),SIZE(YDFIELDS%YRGFL%GFLT1,4)))
ALLOCATE(ZGFLPTS(SIZE(YDFIELDS%YRGFL%GFLPT,1),SIZE(YDFIELDS%YRGFL%GFLPT,2),SIZE(YDFIELDS%YRGFL%GFLPT,3),SIZE(YDFIELDS%YRGFL%GFLPT,4)))
ALLOCATE(ZGFLPTA(SIZE(YDFIELDS%YRGFL%GFLPT,1),SIZE(YDFIELDS%YRGFL%GFLPT,2),SIZE(YDFIELDS%YRGFL%GFLPT,3),SIZE(YDFIELDS%YRGFL%GFLPT,4)))


ALLOCATE(ZSP3DA(SIZE(YDFIELDS%YRSPEC%SP3D,1),SIZE(YDFIELDS%YRSPEC%SP3D,2),SIZE(YDFIELDS%YRSPEC%SP3D,3)))
ALLOCATE(ZSP3DS(SIZE(YDFIELDS%YRSPEC%SP3D,1),SIZE(YDFIELDS%YRSPEC%SP3D,2),SIZE(YDFIELDS%YRSPEC%SP3D,3)))
ALLOCATE(ZSP2DA(SIZE(YDFIELDS%YRSPEC%SP2D,1),SIZE(YDFIELDS%YRSPEC%SP2D,2)))
ALLOCATE(ZSP2DS(SIZE(YDFIELDS%YRSPEC%SP2D,1),SIZE(YDFIELDS%YRSPEC%SP2D,2)))
ALLOCATE(ZSP1DA(SIZE(YDFIELDS%YRSPEC%SP1D,1),SIZE(YDFIELDS%YRSPEC%SP1D,2)))
ALLOCATE(ZSP1DS(SIZE(YDFIELDS%YRSPEC%SP1D,1),SIZE(YDFIELDS%YRSPEC%SP1D,2)))
ALLOCATE(ZDIVA(SIZE(YDFIELDS%YRSPEC%DIV,1),SIZE(YDFIELDS%YRSPEC%DIV,2)))
ALLOCATE(ZDIVS(SIZE(YDFIELDS%YRSPEC%DIV,1),SIZE(YDFIELDS%YRSPEC%DIV,2)))
ALLOCATE(ZVORA(SIZE(YDFIELDS%YRSPEC%VOR,1),SIZE(YDFIELDS%YRSPEC%VOR,2)))
ALLOCATE(ZVORS(SIZE(YDFIELDS%YRSPEC%VOR,1),SIZE(YDFIELDS%YRSPEC%VOR,2)))
ALLOCATE(ZTA(SIZE(YDFIELDS%YRSPEC%T,1),SIZE(YDFIELDS%YRSPEC%T,2)))
ALLOCATE(ZTS(SIZE(YDFIELDS%YRSPEC%T,1),SIZE(YDFIELDS%YRSPEC%T,2)))
ALLOCATE(ZSPA(SIZE(YDFIELDS%YRSPEC%SP,1)))
ALLOCATE(ZSPS(SIZE(YDFIELDS%YRSPEC%SP,1)))
ALLOCATE(ZGMVS(SIZE(YDFIELDS%YRGMV%GMV,1),SIZE(YDFIELDS%YRGMV%GMV,2),SIZE(YDFIELDS%YRGMV%GMV,3),SIZE(YDFIELDS%YRGMV%GMV,4)))
ALLOCATE(ZGMVSS(SIZE(YDFIELDS%YRGMV%GMVS,1),SIZE(YDFIELDS%YRGMV%GMVS,2),SIZE(YDFIELDS%YRGMV%GMVS,3)))
ALLOCATE(ZGMVT1S(SIZE(YDFIELDS%YRGMV%GMVT1,1),SIZE(YDFIELDS%YRGMV%GMVT1,2),SIZE(YDFIELDS%YRGMV%GMVT1,3),SIZE(YDFIELDS%YRGMV%GMVT1,4)))
ALLOCATE(ZGMVT1SS(SIZE(YDFIELDS%YRGMV%GMVT1S,1),SIZE(YDFIELDS%YRGMV%GMVT1S,2),SIZE(YDFIELDS%YRGMV%GMVT1S,3)))
ALLOCATE(ZGMVA(SIZE(YDFIELDS%YRGMV%GMV,1),SIZE(YDFIELDS%YRGMV%GMV,2),SIZE(YDFIELDS%YRGMV%GMV,3),SIZE(YDFIELDS%YRGMV%GMV,4)))
ALLOCATE(ZGMVSA(SIZE(YDFIELDS%YRGMV%GMVS,1),SIZE(YDFIELDS%YRGMV%GMVS,2),SIZE(YDFIELDS%YRGMV%GMVS,3)))
ALLOCATE(ZGMVT1A(SIZE(YDFIELDS%YRGMV%GMVT1,1),SIZE(YDFIELDS%YRGMV%GMVT1,2),SIZE(YDFIELDS%YRGMV%GMVT1,3),SIZE(YDFIELDS%YRGMV%GMVT1,4)))
ALLOCATE(ZGMVT1SA(SIZE(YDFIELDS%YRGMV%GMVT1S,1),SIZE(YDFIELDS%YRGMV%GMVT1S,2),SIZE(YDFIELDS%YRGMV%GMVT1S,3)))
ALLOCATE(ZUBS(SIZE(YDFIELDS%YRSPEC%UB,1)))
ALLOCATE(ZVBS(SIZE(YDFIELDS%YRSPEC%VB,1)))
ALLOCATE(ZUBA(SIZE(YDFIELDS%YRSPEC%UB,1)))
ALLOCATE(ZVBA(SIZE(YDFIELDS%YRSPEC%VB,1)))




DO 
ZDIVS=YDFIELDS%YRSPEC%DIV
ZVORS=YDFIELDS%YRSPEC%VOR
ZTS=YDFIELDS%YRSPEC%T
ZSPS=YDFIELDS%YRSPEC%SP
ZSP3DS=YDFIELDS%YRSPEC%SP3D
ZSP2DS=YDFIELDS%YRSPEC%SP2D
ZSP1DS=YDFIELDS%YRSPEC%SP1D
ZUBS=YDFIELDS%YRSPEC%UB
ZVBS=YDFIELDS%YRSPEC%VB

ZGFLS=YDFIELDS%YRGFL%GFL
ZGFLT1S=YDFIELDS%YRGFL%GFLT1
ZGFLPTS=YDFIELDS%YRGFL%GFLPT

ZGMVS=YDFIELDS%YRGMV%GMV
ZGMVSS=YDFIELDS%YRGMV%GMVS
ZGMVT1S=YDFIELDS%YRGMV%GMVT1
ZGMVT1SS=YDFIELDS%YRGMV%GMVT1S
WRITE(6,*) 'DEBUG CNT4',__LINE__,UBOUND(YDFIELDS%YRSPEC%UB),UBOUND(YDFIELDS%YRSPEC%VB)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SUM(YDFIELDS%YRSPEC%UB),SUM(YDFIELDS%YRSPEC%VB)

!DEBUG BYPASS HYDRO
IF (.TRUE.) THEN
!YDMODEL%YRML_PHY_MF%YRARPHY%LMSE=.FALSE.
LNHEE=.FALSE.
LNHDYN=.FALSE.
CALL SUSI(YDGEOMETRY,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_DYN%YRDYN,YDMODEL%YRML_DYN%YREDYN,NULOUT)
 ...
 &                YDFIELDS%YRSPEC%SP3D,YDFIELDS%YRSPEC%SP2D,YDFIELDS%YRSPEC%SP1D, .TRUE.)
  ...
  IF (NSTOP > 0.OR.LDFI) THEN
    ...
    IF (.NOT.LELAM.OR.NSTOP > 0) THEN
      IF (LNHEE.AND.LSI_NHEE) THEN
        CALL SUNHEESI(YDGEOMETRY,YDRIP,YDDYN,NULOUT,.FALSE.)
      ELSEIF (LNHEE) THEN
        CALL SUNHSI(YDGEOMETRY,YDRIP,YDDYN,YDEDYN,NULOUT,.FALSE.)
      ELSEIF (LNHQE.AND..NOT.LNHQE_SIHYD) THEN
        CALL SUNHQESI(YDGEOMETRY,YDRIP,YDDYN,NULOUT,.FALSE.)
      ELSE
        IF (LELAM) THEN
  ...
  ENDIF
     ...
     !*     3.7bis   Control of time events for IAU
  IF (LELAM.AND.LIAU) THEN
    NSTARTIAU = NINT(TSTARTIAU(NCIAU)/TSTEP)
    IF ((JSTEP == NSTARTIAU).AND.(NCIAU<=NIAU)) THEN
       NSTOPIAU = NINT(TSTOPIAU(NCIAU)/TSTEP)
       CALL ECHGIAU(YDGEOMETRY,YDGFL,YDEPHLI,YDMODEL%YRML_GCONF,YDMODEL%YRML_LBC, &
   & YDMODEL%YRML_DYN%YRDYN,YDSPEC)
    ENDIF
  ENDIF
  ...
  IF (PRESENT(YDFPOS)) THEN
    ...
    ELSE
      LLFPOS=.FALSE.
    ENDIF
    ! post-processing norms computation frequency :
    IF (YDNAMFPIOS%NFRFPDI /= 0) THEN
      YLFPDATA%LLFPDI=LLFPOS.AND.(IFPDITS(JSTEP/YDNAMFPIOS%NFRFPDI) == 1.AND.MOD(JSTEP,YDNAMFPIOS%NFRFPDI) == 0)
    ELSE
      YLFPDATA%LLFPDI=.FALSE.
    ENDIF
  ELSE
    LLFPOS=.FALSE.
    YLFPDATA%LLFPDI=.FALSE.
...
!DEBUG BYPASS FULLPOS STEP IN CNT4
  IF (.FALSE..AND.LLFPOS.AND.((&
!  IF (LLFPOS.AND.((&
...
!DEBUG END BYPASS HYDRO
ENDIF

WRITE(6,*) 'DEBUG CNT4',__LINE__,UBOUND(YDFIELDS%YRSPEC%UB),UBOUND(YDFIELDS%YRSPEC%VB)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SUM(YDFIELDS%YRSPEC%UB),SUM(YDFIELDS%YRSPEC%VB)
ZDIVA=YDFIELDS%YRSPEC%DIV
ZVORA=YDFIELDS%YRSPEC%VOR
ZTA=YDFIELDS%YRSPEC%T
ZSPA=YDFIELDS%YRSPEC%SP
ZSP3DA=YDFIELDS%YRSPEC%SP3D
ZSP2DA=YDFIELDS%YRSPEC%SP2D
ZSP1DA=YDFIELDS%YRSPEC%SP1D

ZGFLA=YDFIELDS%YRGFL%GFL
ZGFLT1A=YDFIELDS%YRGFL%GFLT1
ZGFLPTA=YDFIELDS%YRGFL%GFLPT

ZGMVA=YDFIELDS%YRGMV%GMV
ZGMVSA=YDFIELDS%YRGMV%GMVS
ZGMVT1A=YDFIELDS%YRGMV%GMVT1
ZGMVT1SA=YDFIELDS%YRGMV%GMVT1S

ZUBA=YDFIELDS%YRSPEC%UB
ZVBA=YDFIELDS%YRSPEC%VB

YDFIELDS%YRSPEC%DIV=ZDIVS     
YDFIELDS%YRSPEC%VOR=ZVORS     
YDFIELDS%YRSPEC%T=ZTS     
YDFIELDS%YRSPEC%SP=ZSPS     
YDFIELDS%YRSPEC%SP3D=ZSP3DS
YDFIELDS%YRSPEC%SP2D=ZSP2DS
YDFIELDS%YRSPEC%SP1D=ZSP1DS

YDFIELDS%YRGFL%GFL=ZGFLS
YDFIELDS%YRGFL%GFLT1=ZGFLT1S
YDFIELDS%YRGFL%GFLPT= ZGFLPTS

YDFIELDS%YRGMV%GMV=ZGMVS
YDFIELDS%YRGMV%GMVS=ZGMVSS
YDFIELDS%YRGMV%GMVT1=ZGMVT1S
YDFIELDS%YRGMV%GMVT1S=ZGMVT1SS

YDFIELDS%YRSPEC%UB=ZUBS
YDFIELDS%YRSPEC%VB=ZVBS



!YDMODEL%YRML_PHY_MF%YRARPHY%LMSE=.TRUE.
IF (.TRUE.) THEN!BYPASS NH
LNHEE=.TRUE.
LNHDYN=.TRUE.
IF (LSI_NHEE) THEN
    CALL SUNHEESI(YDGEOMETRY,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_DYN%YRDYN,NULOUT,.TRUE.)
ELSE
    CALL SUNHSI(YDGEOMETRY,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_DYN%YRDYN,YDMODEL%YRML_DYN%YREDYN,NULOUT,.TRUE.)
END IF
...
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRSPEC%DIV,1)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRSPEC%SP3D,1)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRSPEC%SP2D,1)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRSPEC%SP1D,1)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRGFL%GFL,1)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRGMV%GMV,1)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRSPEC%UB,1)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRSPEC%DIV,2)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRSPEC%SP3D,2)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRSPEC%SP2D,2)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRSPEC%SP1D,2)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRGFL%GFL,2)
WRITE(6,*) 'DEBUG CNT4',__LINE__,SIZE(YDFIELDS%YRGMV%GMV,2)
CALL FLUSH(6)

ZALPHA=0.3
YDFIELDS%YRSPEC%DIV=ZDIVA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRSPEC%DIV
YDFIELDS%YRSPEC%VOR=ZVORA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRSPEC%VOR
!YDFIELDS%YRSPEC%T=ZTA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRSPEC%T
!YDFIELDS%YRSPEC%SP=ZSPA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRSPEC%SP
!YDFIELDS%YRSPEC%SP3D=ZSP3DA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRSPEC%SP3D
!YDFIELDS%YRSPEC%SP2D=ZSP2DA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRSPEC%SP2D
!YDFIELDS%YRSPEC%SP1D=ZSP1DA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRSPEC%SP1D

!YDFIELDS%YRGFL%GFL=ZGFLA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRGFL%GFL
!YDFIELDS%YRGFL%GFLT1=ZGFLT1A*ZALPHA+(1-ZALPHA)*YDFIELDS%YRGFL%GFLT1
!YDFIELDS%YRGFL%GFLPT=ZGFLPTA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRGFL%GFLPT

YDFIELDS%YRGMV%GMV=ZGMVA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRGMV%GMV
YDFIELDS%YRGMV%GMVS=ZGMVSA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRGMV%GMVS
YDFIELDS%YRGMV%GMVT1=ZGMVT1A*ZALPHA+(1-ZALPHA)*YDFIELDS%YRGMV%GMVT1
YDFIELDS%YRGMV%GMVT1S=ZGMVT1SA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRGMV%GMVT1S

!YDFIELDS%YRSPEC%UB=ZUBA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRSPEC%UB
!YDFIELDS%YRSPEC%VB=ZVBA*ZALPHA+(1-ZALPHA)*YDFIELDS%YRSPEC%VB
