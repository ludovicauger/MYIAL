...
REAL(KIND=JPRB),ALLOCATABLE :: ZPARBUFS(:,:,:)
TYPE(SURFEX_t),ALLOCATABLE,DIMENSION(:) :: YSCS
INTEGER :: JI
...
ALLOCATE(ZPARBUFS(SIZE(GPARBUF,1),SIZE(GPARBUF,2),SIZE(GPARBUF,3)))


ALLOCATE(YSCS(SIZE(YSURFEX_ARO_ALL,1)))
DO JI=1,SIZE(YSURFEX_ARO_ALL,1)
CALL ALLOCATE_SURFEX_T(YSCS(ji),YSURFEX_ARO_ALL(JI))
ENDDO

WRITE(6,*) 'DEBUG CNT4',__LINE__,UBOUND(GPARBUF)
WRITE(6,*) 'DEBUG CNT4',__LINE__,UBOUND(YSURFEX_ARO_ALL)


DO 

DO JI=1,SIZE(YSURFEX_ARO_ALL,1)
CALL COPY_SURFEX_T(YSCS,YSURFEX_ARO_ALL(JI))
END DO
...
ENDIF !END BYPASS NH
  IF (.NOT. FP_SERV_C001%LFP_SERVER) THEN
    JSTEP = JSTEP + 1
  ENDIF
!YDFIELDS%YRSPEC%DIV=ZDIVA     
!YDFIELDS%YRSPEC%VOR=ZVORA     
!YDFIELDS%YRSPEC%T=ZTA     
!YDFIELDS%YRSPEC%SP=ZSPA     
!YDFIELDS%YRSPEC%SP3D=ZSP3DA
!YDFIELDS%YRSPEC%SP2D=ZSP2DA
!YDFIELDS%YRSPEC%SP1D=ZSP1DA

!YDFIELDS%YRGFL%GFL=ZGFLA
!YDFIELDS%YRGFL%GFLT1=ZGFLT1A
!YDFIELDS%YRGFL%GFLPT= ZGFLPTA

!YDFIELDS%YRGMV%GMV=ZGMVA
!YDFIELDS%YRGMV%GMVS=ZGMVSA
!YDFIELDS%YRGMV%GMVT1=ZGMVT1A
!YDFIELDS%YRGMV%GMVT1S=ZGMVT1SA

!YDFIELDS%YRSPEC%UB=ZUBA
!YDFIELDS%YRSPEC%VB=ZVBA
