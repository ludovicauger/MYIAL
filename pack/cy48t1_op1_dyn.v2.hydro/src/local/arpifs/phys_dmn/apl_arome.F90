...
SUBROUTINE APL_AROME(YDGEOMETRY,YDSURF, YDCFU, YDXFU, YDMODEL, KBL, KGPCOMP, KIDIA , KFDIA , KLON ,&
 ...
 & PGEMU,PGELAM,POROG,PGM,PMU0,PSOLO,PMU0LU,PMU0M,PMU0N,PSIC,PCLON,PSLON,PVO3ABC,PLSM,&
 ...
 & PPBLH_MINFLOTA,PVMAX_THER, PSPSG,PSPSD,PSPSGR,&
...
! PSOLO      : SOLar Obscuration by the Moon.
...
! PPBLH_MINFLOTA: PSEUDO-HISTORICAL ARRAY FOR PBL HEIGHT
! PVMAX_THER : THERMALS MAXIMAL VELOCITY  
! PSPSG      : SNOW WATER EQUIVALENT
! PSPSD      : SNOW DEPTH
! PSPSGR     : SNOW DENSITY
...
!     2021-12, R. Honnert: New diagnostics for light aviation
!     2022-04-13 J.M. Piriou : Sun eclipses (solar obscuration PSOLO).
!     2022-10-10 Y. Seity : add checkmv and checknan
!     2022-11-17 Y. Seity : bf for CMF_CLOUD='STAT'
! End modifications
!-------------------------------------------------------------------------------
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PPBLH_MINFLOTA(KLON)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PVMAX_THER(KLON)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PSPSG(KLON)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PSPSD(KLON)
...
REAL(KIND=JPRB) :: ZTENDSV_TURB_(KFDIA,KLEV,YDMODEL%YRML_GCONF%YGFL%NGFL_EXT) ! Array for Dust modelling
...
REAL(KIND=JPRD) :: ZGEMU_D(KLON)  ! double precision version of PGEMU, for RADACT

! Single scattering albedo of dust (points,lev,wvl) :
REAL(KIND=JPRB) :: ZPIZA_DST_(KLON,KLEV,YDMODEL%YRML_PHY_MF%YRPARAR%NSWB_MNH)
! Assymetry factor for dust (points,lev,wvl) :
REAL(KIND=JPRB) :: ZCGA_DST_(KLON,KLEV,YDMODEL%YRML_PHY_MF%YRPARAR%NSWB_MNH)
! tau/tau_{550} dust (points,lev,wvl) :
REAL(KIND=JPRB) :: ZTAUREL_DST_(KLON,KLEV,YDMODEL%YRML_PHY_MF%YRPARAR%NSWB_MNH)
...
REAL(KIND=JPRB) :: ZQCO2(KLON,KLEV), ZQCH4(KLON,KLEV)
REAL(KIND=JPRB) :: ZQN2O(KLON,KLEV), ZQNO2(KLON,KLEV), ZQC11(KLON,KLEV)
REAL(KIND=JPRB) :: ZQC12(KLON,KLEV), ZQC22(KLON,KLEV), ZQCL4(KLON,KLEV)
REAL(KIND=JPRB) :: ZCHTIX(KLON,KLEV+1), ZCAPH(KLON,KLEV+1), ZTH(KLON,KLEV+1)
REAL(KIND=JPRB) :: ZDUM(KLON,KLEV), ZGELAM(KLON)

REAL(KIND=JPRB) :: ZTKEM(KLON,KLEV)
...
REAL(KIND=JPRB) :: ZFLXZTHV(KLON,KLEV)
...
REAL(KIND=JPRB) :: ZTP2(KLON), ZWS2(KLON), ZWP2(KLON), ZWSI2(KLON), ZWPI2(KLON)
REAL(KIND=JPRB) :: ZWR2(KLON), ZSNA2(KLON)
...
! Bof. REK
REAL(KIND=JPRB) :: ZMULNOISE(KLON,1)
REAL(KIND=JPRB) :: ZDUMMY(KLON,KLEV)
REAL(KIND=JPRB) :: ZDUMMY1(KLON)
REAL(KIND=JPRB) :: PTENDENCYU(KLON,KLEV,1),PTENDENCYV(KLON,KLEV,1),PTENDENCYT(KLON,KLEV,1),PTENDENCYQ(KLON,KLEV,1)
...
ASSOCIATE(MINPRR=>YDPARAR%MINPRR, MINPRS=>YDPARAR%MINPRS, MVQS=>YDPARAR%MVQS, &
 ...
 & LIMIT_TENDQ=>YDPARAR%LIMIT_TENDQ, &
 & LBL89ARPEGE=>YDPARAR%LBL89ARPEGE,LBL89EXP=>YDPARAR%LBL89EXP, &
 & LPROJQITURB=>YDPARAR%LPROJQITURB,LGZ=>YDPARAR%LGZ,&
 ...
 & LRDUST=>YDARPHY%LRDUST, NGRADIENTS=>YDARPHY%NGRADIENTS, &
 & LRDEPOS=>YDARPHY%LRDEPOS, LRCO2=>YDARPHY%LRCO2, LMSE=>YDARPHY%LMSE, &
 ...
 & LGCHECKMV=>YDPHY%LGCHECKMV, LGCHECKNAN=>YDPHY%LGCHECKNAN,&
 ...
 & LFLEXDIA=>YLDDH%LFLEXDIA, LRSLDDH=>YLDDH%LRSLDDH, &
 & RDECLI=>YDRIP%RDECLI, &
 & RCODEC=>YDRIP%RCODEC, &
 & RHGMT=>YDRIP%RHGMT, &
 & RSIDEC=>YDRIP%RSIDEC, &
 ...
 & LXCLPMF=>YDXFU%LXCLPMF,&  
 & LXVMAXTH=>YDXFU%LXVMAXTH,&
 & LFLASH =>YDCFU%LFLASH,&
...
IF (LDXFUMSE) THEN
  ZDTMSE=0.01_JPRB
  ZSTATI=RSTATI-ZDTMSE*0.5_JPRB
  ZADTMS=0._JPRB
ELSE
!DEBUG PUT PDT=0. HERE
  ZDTMSE=PDT
  ZSTATI=RSTATI
  ZADTMS=ZDTMSE
ENDIF
...
IF(LTWOTL) THEN
  ZRHGMT=REAL(RHGMT,JPRB)-ZDTMSE*0.5_JPRB
ELSE
  ZRHGMT=REAL(RHGMT,JPRB)
ENDIF

!DEBUG ADD LNHDYN
LLMSE=LMSE.AND.(NSURFEXCTL >= 2)!.AND.LNHDYN
LLMSE_PARAM=LLMSE
...
IF (INIT0 >= 0) THEN
  ...
  PFRSOPS(:)=ZVALUE
  !PVMAX_THER(:)=ZVALUE
  !PPBLH_MINFLOTA(:)=ZVALUE 
  PDIAGH(:)=ZVALUE

  ZTENDSV_TURBLIMA_(:,:,:)=ZVALUE
  IF (LRDUST) ZTENDSV_TURB_(:,:,:)=ZVALUE
...
DO JLEV=1, KLEV
  DO JLON = KIDIA, KFDIA
    ZTKEM(JLON,JLEV)=MAX(PTKEM(JLON,JLEV),PPTKEMIN)
  ENDDO
ENDDO
...
IF (LMICRO) THEN
  ...
  IF (LMFSHAL) THEN
    ...
    IF (KSTEP==0.OR.CMF_CLOUD=='STAT') THEN
      DO JLEV = 1, KLEV 
...
IF (LRDUST) THEN
  ...
  CALL ARO_MNHDUST (IKL,KFDIA,KLEV,NGFL_EXT, PDT,ZSVMIN_,ZZZ_,ZDZZ_,&
   ...
   & NSWB_MNH,KSTEP+1,ZSVM_,ZPIZA_DST_(KIDIA:KFDIA,:,:),ZCGA_DST_(KIDIA:KFDIA,:,:),ZTAUREL_DST_(KIDIA:KFDIA,:,:),&
   & ZAERD_,IEZDIAG_CHEM,&
...
!              ZALBP(JLON,JSW)=PALBIN(JLON)+ZSALBCOR
  ...
  ELSE
    ...
    ELSE
      ...
      ENDDO
      ! Init ZCHTIX
      ! Warning PAPRSM is 0:KLEV here whereas in radintg it is
      ! 1:KLEV+1
      DO JK=2,KLEV
        ZCAPH(KIDIA:KFDIA,JK)=PAPRSM(KIDIA:KFDIA,JK-1)
        ZTH(KIDIA:KFDIA,JK)=(ZTM(KIDIA:KFDIA,JK-1)*PAPRSFM(KIDIA:KFDIA,JK-1)&
      & *(PAPRSFM(KIDIA:KFDIA,JK)-PAPRSM(KIDIA:KFDIA,JK-1))&
      & +ZTM(KIDIA:KFDIA,JK)*PAPRSFM(KIDIA:KFDIA,JK)*(PAPRSM(KIDIA:KFDIA,JK-1)&
     & -PAPRSFM(KIDIA:KFDIA,JK-1)))&
     &*(1.0_JPRB/(PAPRSM(KIDIA:KFDIA,JK-1)*(PAPRSFM(KIDIA:KFDIA,JK)-PAPRSFM(KIDIA:KFDIA,JK-1))))
        ZCHTIX(KIDIA:KFDIA,JK)=ZTH(KIDIA:KFDIA,JK)
      ENDDO
      ! QUANTITIES AT BOUNDARIES
      ZCAPH(KIDIA:KFDIA,1)=PAPRSM(KIDIA:KFDIA,0)
      ZCAPH(KIDIA:KFDIA,KLEV+1)=PAPRSM(KIDIA:KFDIA,KLEV)
      ZTH(KIDIA:KFDIA,1)=ZTM(KIDIA:KFDIA,1)-PAPRSFM(KIDIA:KFDIA,1)*(ZTM(KIDIA:KFDIA,1)&
       & -ZTH(KIDIA:KFDIA,2))/(PAPRSFM(KIDIA:KFDIA,1)-PAPRSM(KIDIA:KFDIA,1))
      ZTH(KIDIA:KFDIA,KLEV+1)=ZTSURF(KIDIA:KFDIA)
      ZCHTIX(KIDIA:KFDIA,KLEV+1)=ZTH(KIDIA:KFDIA,KLEV+1)
      ZCHTIX(KIDIA:KFDIA,1)=ZTH(KIDIA:KFDIA,1)
      ZGEMU_D=REAL(PGEMU,JPRD)
      CALL RADACT(YDMODEL%YRML_PHY_RAD%YREAERD,YDERAD,YDMODEL%YRML_PHY_AER%YREAERSNK,YDRIP, &
         & KIDIA, KFDIA, KLON, KLEV,&
         & 1 , KLON , KLON  ,0 , 1,&
         & PAPRSM , ZGELAM, ZGEMU_D, PCLON , PSLON, ZCHTIX,&
         & ZQVM   , ZQSAT  , ZDUM  ,&
         & ZRAER  , ZAERO,ZROZ  )
      DO JAE=1,6
        DO JK=1,KLEV
  ...
  IF (NOZOCL==3.OR.NOZOCL==4) THEN ! Clims MACC
     CALL RADGHG ( YDERAD,YDRIP,KIDIA,KFDIA,KLON,KLEV,KLON,PAPRSM,PGEMU,&
...
IF (LRAYFM) THEN
   ...
   IF (MOD(KSTEP,NRADFR) == 0) THEN 
     ...
     & KIDIA   , KFDIA   , KLON  , KLEV     ,NOZOCL   , & 
     & NAERMACC, IAERO   , ZALBD , ZALBP    , PAPRSM  , PAPRSFM , &
     & PCLFS   , ZQO3    , ZQCO2 , ZQCH4    , ZQN2O   , &
     & ZQNO2   , ZQC11   , ZQC12 , ZQC22    , ZQCL4   , &
     & ZAER    , ZAERO   , PDELPM, ZEMIS    , &
   ...
   & PAPRSM  , ZEMIS  , PEMTD  , PMU0, PSOLO, ZQVM,&
  ...
  DO JLON = KIDIA, KFDIA
    ...
    PSDUR(JLON)=PSDUR(JLON)+ZSDUR(JLON)*ZADTMS ! fix stepx case
...
CALL FLUSH(6)
    CALL ARO_GROUND_DIAG( KBL, KGPCOMP,&
       ...
       & ZSSO_STDEV_, PSPSG(KIDIA:KFDIA),PSPSD(KIDIA:KFDIA),&
       & ZBUDTH_, ZBUDSO_,&
       & ZFCLL_, ZTOWNS_,&
       & ZCD_, PSIC(KIDIA:KFDIA))
    ...
    CALL ARO_GROUND_DIAG_2ISBA( KBL, KGPCOMP, &
       ...
       & ZTP2(KIDIA:KFDIA), ZWS2(KIDIA:KFDIA), ZWP2(KIDIA:KFDIA), &
       & ZWSI2(KIDIA:KFDIA), ZWPI2(KIDIA:KFDIA), ZWR2(KIDIA:KFDIA), &
       & ZDUMMY1(KIDIA:KFDIA), PSPSGR(KIDIA:KFDIA), ZDUMMY1 )
 
  ELSE

    ZBUDTH_(KIDIA:KFDIA)=0._JPRB
...
!!$  ENDIF
  ...
  IF (LRDUST) THEN
    ! Appel avec les arguments modifiés pour variables Dust :
    ...
    CALL ARO_TURB_MNH(KKA=IKA,KKU=IKU,KKL=IKL,KLON=KFDIA,KLEV=KLEV,&
     ...
     & KSV=NGFL_EXT,KTCOUNT=KSTEP+1,KGRADIENTS=NGRADIENTS,LDHARATU=LHARATU,&
     & OPROJQI=LPROJQITURB, PTSTEP=ZDT,&
  ...
  ! KSV_TURB, ZSFTURB, ZTURBM, ZTURBS, ZTENDSV_TURB
    CALL ARO_TURB_MNH(KKA=IKA,KKU=IKU,KKL=IKL,KLON=KFDIA,KLEV=KLEV,&
     & KRR=NRR, KRRL=NRRL,KRRI= NRRI,&
     & KSV=NLIMA,KTCOUNT=KSTEP+1,KGRADIENTS=NGRADIENTS,LDHARATU=LHARATU,&
     & OPROJQI=LPROJQITURB, PTSTEP=ZDT,&
     & PZZ=ZZZ_,PZZF=ZZZ_F_,&
     & PZZTOP=ZZTOP_,PRHODJ=ZRHODJM__,PTHVREF=ZTHVREFM__,&
     & PRHODREF=ZRHODREFM__,HINST_SFU='M',&
     ...
     & YDLDDH=YDMODEL%YRML_DIAG%YRLDDH,YDMDDH=YDMODEL%YRML_DIAG%YRMDDH)
  ENDIF
IF (LXVMAXTH .OR. LXCLPMF) THEN
  ZFLXZTHV(:,:)=HUGE(1._JPRB)
  DO JLEV = 1,KLEV
     DO JLON = KIDIA,KFDIA
        ZFLXZTHV(JLON,JLEV)=ZFLXZTHVMF_SUM__(JLON,JLEV)
     ENDDO
  ENDDO
  CALL AROTHERDIA(YDTOPH,YDPARAR,KIDIA,KFDIA,KLON,KLEV, &
    & ZFLXZTHV, PAPHIFM,POROG,&
    & PPBLH_MINFLOTA,PVMAX_THER)
ENDIF
...
IF(YSPPT_CONFIG%LSPSDT) THEN
  ...
  CALL SPPTEN (YDMODEL%YRML_PHY_EC%YRECLDP,YGFL, &
   ...
   & PPHY_U=PTENDENCYU,PPHY_V=PTENDENCYV,PPHY_T=PTENDENCYT,PPHY_Q=PTENDENCYQ, & ! In: (u,v,t,qv) tendencies to perturb
...
IF (LIMIT_TENDQ)  THEN
  DO JLEV=1,KLEV
    DO JLON=KIDIA,KFDIA
       PTENDR(JLON,JLEV,1)=MAX(PTENDR(JLON,JLEV,1),&
                          &(-PQVM(JLON,JLEV)+1.E-12_JPRB)/TSTEP)
    ENDDO
  ENDDO
ENDIF
...
!IF (LSURFEX) THEN ! OK 15:28
!END BYPASS
!ENDIF
IF (LFLEXDIA) THEN
      !   surface variables
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PLSM,'SVLSM',YDDDH,CDTYPE='V')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTSURF,'SVTS',YDDDH,CDTYPE='V')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PSPSG,'SVWN',YDDDH,CDTYPE='V')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PTCLS,'SVTCLS',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PQCLS,'SVQCLS',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PHUCLS,'SVHUCLS',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PUCLS,'SVUCLS',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PVCLS,'SVVCLS',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PPBLH,'SVPBLH',YDDDH,CDTYPE='S')
      ZTMP(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*ZWS2(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP,'SVWS',YDDDH,CDTYPE='V')
      ZTMP(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*ZWP2(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP,'SVWP',YDDDH,CDTYPE='V')
      ZTMP(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*ZWSI2(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP,'SVWIS',YDDDH,CDTYPE='V')
      ZTMP(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*ZWPI2(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP,'SVWIP',YDDDH,CDTYPE='V')
      ZTMP(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*ZTP2(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP,'SVTP',YDDDH,CDTYPE='V')
      ZTMP(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*PSPSG(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,POROG,'SVOROG',YDDDH,CDTYPE='S')
      ZTMP(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*PFEVL(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP,'SFEVAPLIQ',YDDDH,CDTYPE='F')
      ZTMP(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*PFEVN(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP,'SFEVAPNEG',YDDDH,CDTYPE='F')
      ZTMP(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*PFCLL(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP,'SFCHLATLI',YDDDH,CDTYPE='F')
      ZTMP(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*PFCLN(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP,'SFCHLATNE',YDDDH,CDTYPE='F')
      ZTMP(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*PFCS(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP,'SFCHSENS',YDDDH,CDTYPE='F')
      !set to 0._JPRB
      ! ZTMP(KIDIA:KFDIA)=0._JPRB
      ...
      ! surface radiation
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PFRSO(KIDIA:KFDIA,KLEV,1),'SFRAYSO',YDDDH,CDTYPE='F')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PFRTH(KIDIA:KFDIA,KLEV,1),'SFRAYTH',YDDDH,CDTYPE='F')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PFRSODS(KIDIA:KFDIA),'SFRAYSODS',YDDDH,CDTYPE='F')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PFRTHDS(KIDIA:KFDIA),'SFRAYTHDS',YDDDH,CDTYPE='F')
      ZTMP(KIDIA:KFDIA)=1_JPRB-PFRSO(KIDIA:KFDIA,KLEV,1)/MAX(ZEPSNEB,PFRSODS(KIDIA:KFDIA))
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP,'SVALB',YDDDH,CDTYPE='S')
      ! surface precipitations
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PFPLSL(KIDIA:KFDIA,KLEV),'SFPRELIGE',YDDDH,CDTYPE='F')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PFPLSN(KIDIA:KFDIA,KLEV),'SFPRENEGE',YDDDH,CDTYPE='F')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PFPLSG(KIDIA:KFDIA,KLEV),'SFPREGRPL',YDDDH,CDTYPE='F')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PFPLSH(KIDIA:KFDIA,KLEV),'SFPREHAIL',YDDDH,CDTYPE='F')
      ! surface wind stress
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PSTRTU(KIDIA:KFDIA,KLEV),'SFUUTUR',YDDDH,CDTYPE='F')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PSTRTV(KIDIA:KFDIA,KLEV),'SFVVTUR',YDDDH,CDTYPE='F')

     ! WRITE(NULOUT,*) 'LFLEXDIA ARPEGE WITH NTOTSURF = ',NTOTSURF,&
     !    & ' AND NTOTSVAR = ',NTOTSVAR, ' AND NTOTSVFS = ',NTOTSVFS
     ! 3D Variables :
      ZTMP2(KIDIA:KFDIA,:)=ZFPR(KIDIA:KFDIA,:,2)+ZFPR(KIDIA:KFDIA,:,3)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMP2(:,:),'FQTPRECISTL',YDDDH)
      ZTMP2(KIDIA:KFDIA,:)=ZFPR(KIDIA:KFDIA,:,4)+ZFPR(KIDIA:KFDIA,:,5)
...
IF (LFLASH) THEN
  ...
  CALL DIAGFLASH(YDCFU,KIDIA,KFDIA,KLON,KLEV,KSTEP,&
    ...
    &PDELPM,ZAPHIFM,ZTM,PWM,PLSM,PFLASH)
ENDIF
!
...
!IF (LSURFEX) THEN ! OK 15:35
  !-------------------------------------------------
  ! Check if fluxes/tendencies are different from NaN.
  !-------------------------------------------------

IF(LGCHECKNAN) THEN
CALL CHECKNANARO(YDRIP,YDPHY0,YDPHY2,KIDIA,KFDIA,KLON,NRR,KLEV,&
  &KSTEP,PAPHIM,PAPHIFM,PAPRSM,PAPRSFM,PGELAM,PGEMU,PMU0,PLSM,PTM,PQVM,ZQSAT,&
  &ZTSURF,PTENDT, PTENDR, PTENDU, PTENDV,PTENDW,PTENDTKE,&
  &PFRTH(:,:,1), PFRSO(:,:,1),PFRTHDS, PFRSODS,&
  &PCLCH,PCLCL,PCLCM,PCLCT,PFPLSL,PFPLSN,PFPLSG,PSTRTU,PSTRTV,&
  &PFCS(:,1),PFCLL(:,1),PFCLN(:,1),PFEVL(:,1),PFEVN(:,1))
ENDIF 
